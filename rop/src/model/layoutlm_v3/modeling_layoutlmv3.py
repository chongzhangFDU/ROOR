...
class LayoutLMv3ForRORelation(LayoutLMv3PreTrainedModel):
    ...
    def __init__(self, config, head_size=128, dropout=0.1):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
        self.layoutlmv3 = LayoutLMv3Model(config)
    ...
    def forward(self,
        ...
        # 通过编码器，得到token表示sequence_output
        ...
        # mean_features(units特征）: (bs, num_units, 768)
        # 计算方式：使用unit_masks来加权sequence_output中的元素，得到每个unit的特征，作为global pointer输入
        # 1. 根据unit_masks加权求和每个单元中的特征，注意维度
        masked_sequence_output = sequence_output.unsqueeze(1) * unit_masks.unsqueeze(-1)
        summed_features = masked_sequence_output.sum(dim=2)  # [batch_size, num_units, feature_size]
        # 2. 计算每个单元中的特征均值（使用clamp避免全零的unit_mask）
