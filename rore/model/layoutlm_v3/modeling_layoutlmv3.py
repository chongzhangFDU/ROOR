...
class LayoutLMv3SelfAttention(nn.Module):
    def __init__(self, config, lam=None):
        super().__init__()
        ...
        self.lam = None if lam is None else nn.Parameter(torch.tensor(lam)) 
    ...
    def forward(
        ...
        if ro_attn is not None and self.lam is not None:
            attention_scores += self.lam * ro_attn.unsqueeze(1)
...
class LayoutLMv3Attention(nn.Module):
    def __init__(self, config, lam=None):
        super().__init__()
        self.self = LayoutLMv3SelfAttention(config, lam)
...
class LayoutLMv3Layer(nn.Module):
    def __init__(self, config, lam=None):
        ...
        self.seq_len_dim = 1
        self.attention = LayoutLMv3Attention(config, lam)
...
class LayoutLMv3Encoder(nn.Module):
    def __init__(self, config, detection=False, out_features=None, 
                 lam=None, num_ro_layers=None):
        super().__init__()
        self.config = config
        self.num_ro_layers = num_ro_layers
        self.detection = detection
        self.layer = nn.ModuleList([LayoutLMv3Layer(config, lam) for _ in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            # 辅助信号融入
            if self.num_ro_layers is not None and i >= self.num_ro_layers: ro_attn = None
...
class LayoutLMv3Model(LayoutLMv3PreTrainedModel):
    ...
    def __init__(self, config, detection=False, out_features=None, image_only=False, 
        ...
        self.encoder = LayoutLMv3Encoder(
            config, detection=detection, out_features=out_features, 
            lam=lam, num_ro_layers=num_ro_layers)
